...
reset_handler:
    @ -----------------------------------------------------------------------
    @ 0. Flush and invalidate caches to ensure coherency after U-Boot load.
    @    U-Boot may have loaded us via DMA with D-cache enabled. Without
    @    this, literal pool reads and instruction fetches may see stale data.
    @ -----------------------------------------------------------------------
    @ Invalidate entire I-cache
    mov   r0, #0
    mcr   p15, 0, r0, c7, c5, 0    @ ICIALLU — invalidate all I-cache

    @ Invalidate and flush entire D-cache (clean + invalidate)
    @ For Cortex-A8, iterate over all sets and ways
    @ Invalidate I-cache only — sufficient since we disable D-cache immediately
    mov   r0, #0
    mcr   p15, 0, r0, c7, c5, 0    @ ICIALLU
    mcr   p15, 0, r0, c7, c10, 4   @ DSB
    mcr   p15, 0, r0, c7, c5, 4    @ ISB

    @ Disable caches and MMU, clear V bit
    mrc   p15, 0, r0, c1, c0, 0
    bic   r0, r0, #(1 << 13)       @ V bit — use VBAR
    bic   r0, r0, #(1 << 12)       @ I — disable I-cache
    bic   r0, r0, #(1 << 2)        @ C — disable D-cache
    bic   r0, r0, #(1 << 0)        @ M — disable MMU
